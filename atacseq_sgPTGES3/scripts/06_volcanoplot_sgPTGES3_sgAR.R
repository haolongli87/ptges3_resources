...
getVolcanoPlot <- function(df, genes, logfc.threshold, pvalue.threshold, filterby.pvalueadj, fc.upperbound, nlogp.upperbound){
	...
	p <- ggplot(df, aes(x=log2FoldChange, y=nlogp, label=Label)) + 
			geom_hline(yintercept = yval, colour="gray70", linetype=4, linewidth=0.25, alpha=0.7) +
			geom_vline(xintercept = xval, colour="gray70", linetype=4, linewidth=0.25, alpha=0.7) +		
			geom_vline(xintercept = -xval, colour="gray70", linetype=4, linewidth=0.25, alpha=0.7) +
...
p <- getVolcanoPlot(df=dat, genes=genes,
        ...
        fc.upperbound=3, nlogp.upperbound=8)

### GENERATE PLOT ---
file.plot <- file.path(dir.reproduce_plots, "volcano_plot_sgPTGES3_sgAR.pdf")
pdf(file.plot, height=3, width=3)
    grid.arrange(p, nrow=1, ncol=1)
dev.off()	

### VOLCANO PLOT ---
p2 <- getVolcanoPlot(df=dat, genes=NA,
        logfc.threshold=1, pvalue.threshold=0.05, filterby.pvalueadj=TRUE, 
        fc.upperbound=3, nlogp.upperbound=8)


file.plot <- file.path(dir.reproduce_plots, "volcano_plot_sgPTGES3_sgAR_notext.pdf")
