...
# This will also analyze heat shock pathways
set.seed(1)
##################################################################
library(stringr)
library(dplyr)
...
# Load data files
##################
fn_gene_count <- paste0(data_dir, "raw_gene_counts_matrix_gencode_v30.PTGES3_inhibitor.txt")
# The raw gene counts can be downloaded from our GEO deposit

fn_hallmark_pathways <- paste0(data_dir, "h.all.v6.2.symbols.gmt")
fn_ensemblinfo <- paste0(data_dir, "gencode.v30_gene_annotation_table.txt")
...
sample_info$condition <- c("DMSO", "DMSO", "rac18", "rac18")
...
dds <- dds[keep,]
dds$condition <- factor(dds$condition, levels = c("DMSO", "rac18"))
...
summary <- summary[with(summary, order(padj)),]

summary$GeneSymbol <- ensemblinfo$GeneSymbol[match(rownames(summary), rownames(ensemblinfo))]
...
colnames(normalized_counts_log2) <- c("DMSO_rep1", "DMSO_rep2", "rac18_rep1", "rac18_rep2")

normalized_counts_mean <- data.frame(DMSO_mean = rowMeans(normalized_counts[, c(1,2)]),
                                     rac18_mean = rowMeans(normalized_counts[, c(3,4)]))
normalized_counts_mean$DMSO_mean_log2 <- log2(normalized_counts_mean$DMSO_mean + 1)
normalized_counts_mean$rac18_mean_log2 <- log2(normalized_counts_mean$rac18_mean + 1)
normalized_counts_mean$geneName <- rownames(normalized_counts)

################################################################################
# Volcano plot highlighting AR pathway genes
################################################################################
fn_volcano <- paste0(output_dir, "volcano_plot_RNAseq_rac18_DMSO_LNCaP.pdf")
# highlight AR pathway genes significantly downregulated + AR
...
label_df_ar_genes <- summary_filtered[which((str_detect(summary_filtered$sig, "p") & 
                                               summary_filtered$GeneSymbol %in% ar_genes) | summary_filtered$GeneSymbol == "AR" ),]
volc = ggplot(summary_filtered, aes(log2FoldChange, -log10(pvalue))) + #volcanoplot with log2Foldchange versus pvalue
  ...
  geom_text_repel(data=label_df_ar_genes,
                  aes(label=GeneSymbol), size = 4) + #adding text AR genes
...
################################################################################
# GSEA showing AR signaling pathway is top downregulated upon rac18 treatment
################################################################################
fn_gsea_nes <- paste0(output_dir, "GSEA_RNAseq_rac18_DMSO_LNCaP_NES.pdf")
...
################################################################################
fn_heatmap_all_genes_ar_pathway <- paste0(output_dir, "Heatmap_all_genes_HALLMARK_AR_rac18_DMSO.pdf")
fn_heatmap_de_genes_ar_pathway <- paste0(output_dir, "Heatmap_DE_genes_HALLMARK_AR_rac18_DMSO.pdf")
...
pdf(fn_heatmap_de_genes_ar_pathway, width = 4.5, height = 4.5)
...
# fn_de_genes <- paste0(output_dir, "DE_genes.csv")
...
##############################################
# NG responses:
# GSEA showing Heat shock protein pathways are not enriched
################################################################################
genes_rank <- summary_filtered$stat
names(genes_rank) <- summary_filtered$GeneSymbol
genes_rank <- genes_rank[names(genes_rank)[!duplicated(names(genes_rank))]]
...
fn_fgsea <- paste0(hsp_output_dir, "fgsea_hsp_rac18_DMSO.xlsx")
write.xlsx(fgseaRes_hsp, fn_fgsea)

# Per Luke request, save all pathways GSEA result
...
fn_fgsea_all_pathways <- paste0(hsp_output_dir, "fgsea_all_pathways_rac18_DMSO.xlsx")
write.xlsx(fgseaRes_all_pathways, fn_fgsea_all_pathways)

# Save a table of DESeq result
fn_deseq2_result <- paste0(hsp_output_dir, "DESeq2_result_rac18_DMSO.csv")
fn_deseq2_result_hsp_genes <- paste0(hsp_output_dir, "DESeq2_result_HSP_rac18_DMSO.csv")

summary_filtered_ranked <- summary_filtered[with(summary_filtered, order(stat)),]
write.csv(summary_filtered_ranked, fn_deseq2_result)

genes_in_enriched_hsp_pathways <- unique(unlist(fgseaRes_hsp$leadingEdge[fgseaRes_hsp$padj < 0.05]))
write.csv(summary_filtered_ranked[which(summary_filtered_ranked$GeneSymbol %in% genes_in_enriched_hsp_pathways),],
          fn_deseq2_result_hsp_genes)


###################################################################
# NG responses: 
# Plot the mean counts for rac18 and DMSO for the HSF1 genes
###################################################################
fn_cor_plot <- paste0(hsp_output_dir, "cor_DMSO_rac18_REACTOME_HSF1_ACTIVATION_genes.pdf")
...
min <- min(c(normalized_counts_mean$DMSO_mean_log2[r_hsf_genes], normalized_counts_mean$rac18_mean_log2[r_hsf_genes]))
max <- max(c(normalized_counts_mean$DMSO_mean_log2[r_hsf_genes], normalized_counts_mean$rac18_mean_log2[r_hsf_genes]))

plot <- ggplot(data = normalized_counts_mean[r_hsf_genes,], 
               aes(x = DMSO_mean_log2, y = rac18_mean_log2)) + geom_point() + 
  geom_abline(slope = 1,intercept = 0, color = "blue", size = 1) +
  xlim(min, max) + ylim(min, max) +
  xlab("DMSO log2 normalized gene counts") +
  ylab("rac18 log2 normalized gene counts") +
